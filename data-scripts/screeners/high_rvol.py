...
cursor = conn.cursor()
cursor.execute('delete from rvol_seperated')
...
for i in range(0, len(tickers), 200):
    ...
    for j in range(len(daily)):
        try:
            ...
            if ohlc_df['atr_pct'].iloc[-1] < .03:
            ...
            elif ohlc_df['volume'].iloc[-1] / ohlc_df['avg_vol'].iloc[-1] < 3.0:
                pass
            
            else:
                current_rvol = ohlc_df['volume'].iloc[-1] / ohlc_df['avg_vol'].iloc[-1]
                ...
                if total > 0 and not isnan(current_atr):
                    ...
                    rvol[current_ticker] = current_rvol
                    print(f'rvol: {current_rvol}\n')
...
for key in atrs.keys():
    cursor.execute('insert into rvol_seperated (ticker, side, atr, rvol, score) \
                   values (%s, %s, %s, %s, %s)', (key, seperated[key], atrs[key], rvol[key], scores[key]))
